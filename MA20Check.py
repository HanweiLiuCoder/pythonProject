...
#    arr_tupo250 =[]  #MACD金叉过后，突破大平台
    ...
    #for i in range(1,10):
        try:
            ...
            #股价离20日线接近，而且前20日有过涨停
            if (float(daily['close'].tail(1)) > float(daily['MA20'].tail(1))) and (float(daily['close'].tail(1)) - float(daily['MA20'].tail(1))) / float(daily['MA20'].tail(1)) < 0.01 :
                #check 是否有过涨停
                cnt_stop = 0
                for i in range(1,20):
                    if float(daily['pct_chg'].tail(i)) > 0.95 :
                        cnt_stop += 1
                if cnt_stop > 1:
                    print("{0} 接近20日均线，前面有过 {1} 次涨停".format(tscode,cnt_stop))
