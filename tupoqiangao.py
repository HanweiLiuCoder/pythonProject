...
if __name__ == '__main__':
    ...
    #选股记录，写入到文件：
    filename = "D:\\Investment\\DailyData\\" + str(datetime.date.today()) + ".txt"
...
#    arr_tupo250 =[]  #MACD金叉过后，突破大平台
    ...
    #stock_code_list = ['000513','300016']  #东方财富

    stock_data = ak.stock_zh_a_spot_em()
    stock_code_list = stock_data[stock_data["涨跌幅"] > 5.0]["代码"].unique()
    ...
    #for i in range(1,10):
        try:
            ...
            if today_pct < 0.05:
                continue
            #comment += ' | 当日涨幅{}%'.format(round(float(today_pct)*100,0))
            ...
            # if tupodays > 200: #155天新高的股票才有意义去看期间是否有放量
            ...
            # 期间有过2倍放量
            # 当日涨幅超过5%
            if vol_pct > 1.5 and tupodays > 60 and (np.array(vol_list[m-tupodays:m-1]) > 2.0).all():
                comm = tscode+ " | 放量突破前高！" + " | 突破{}天".format(tupodays) +' | 当日涨幅{}%'.format(round(float(today_pct)*100,0))
                print(comm)
                arr_tupo.append(comm)
            ...
            #
            # if vol_times >= 10.0:  #期间的量能有过超越均值两倍说明有过放量
            #     comment += ' | 期间有过{}倍以上的放量现象\n'.format(vol_times)
            # else:
            #     comment += '\n'

            # close_list = list(daily['close'])
            # list_pct_chg = list(daily['pct_chg'])  # 涨幅
            #
            # # 判断期间涨停次数
            # cnt = 0
            # for p in list_pct_chg[0 - tupodays: -1]:
            #     if p > 0.09:  # 涨幅超过9%,视为涨停
            #         cnt = cnt + 1
            # if cnt > 1:
            #     comment += ' | 期间涨停次数：{}\n'.format(cnt)
            #
            # maxclose = max(close_list[0 - tupodays: -1])
            # minclose = min(close_list[0 - tupodays: -1])
            # if (maxclose - minclose)/minclose < 1 and cnt>1:  #涨幅超过1倍的股票暂时不考虑 没有过涨停的股票不考虑
            #     #检查是否有过涨停，到目前位置涨幅多少
            #     print(comment)
            #     file.write(comment)
            #     arr_tupo[tscode]=[tupodays,comment]
    ...
    #
    # #按照突破天数排序之后输出：
    # sorted_dict = sorted(arr_tupo.items(), key=lambda item: item[1][0])
    # for i in sorted_dict:
    #     print(i[0],' 突破周期：', i[1])
    #     file.write(i[0]+' 突破周期：'+str(i[1][0])+"\n")
    #
    # file.write('\n-----------done--------------\n')
    # file.close()  # 关闭文件

    #for key, values in sorted_dict.items():
    #    print(key,": 突破周期",values)
